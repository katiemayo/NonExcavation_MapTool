...
class Korweb:
	def __init__(self, username, password, customerid, web_driver_path, ticket, output_path, laptop, sender):
		...
		self.driver.find_element_by_id("CustomerId").send_keys("MINNEAPOLIS")
		self.driver.find_element_by_id("customer-redirect-submit-btn").click()
		time.sleep(TimeSleep)
		self.driver.find_element_by_id("username").send_keys("SWSENGINEER")
		self.driver.find_element_by_id("password").send_keys("SWSLocate123!")
		time.sleep(TimeSleep)
